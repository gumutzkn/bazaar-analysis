...
router.get('/getAll', async (req, res) => {
	try {
		const [datas, uniqueMalAdiValues] = await Promise.all([
			Model.find(),
			Model.distinct('MAL_ADI'),
		]);
...
router.get('/getcompared/:mal_adi/:yil1/:ay1/:yil2/:ay2', async (req, res) => {
	try {
		...
		if (ay1 > 12 || ay2 > 12) {
			...
			const yuzdelikFark =
				((Aylik_Ortalama_Ucret2 - Aylik_Ortalama_Ucret1) / Aylik_Ortalama_Ucret1) * 100;

			let yorum = '';
			if (yuzdelikFark > 0) {
				yorum = `${ay2} - ${yil2} fiyatı, ${ay1} - ${yil1} fiyatıdan %${yuzdelikFark.toFixed(
					2,
				)} daha yüksek. Bu, fiyatların arttığını gösteriyor.`;
			} else if (yuzdelikFark < 0) {
				yorum = `${ay2} - ${yil2} fiyatı, ${ay1} - ${yil1} fiyatından %${Math.abs(
					yuzdelikFark,
				).toFixed(2)} daha düşük. Bu, fiyatların düştüğünü gösteriyor.`;
			} else {
